...
const CustomBarLabel = (props) => {
  ...
  const isCritical = index === 0; // Billing is first item
  // Calculate full track width (background bar extends to max domain of 50)
  const trackWidth = width * (50 / value);
  return (
    <g>
      ...
      {isCritical && (
        <svg x={x + trackWidth + 8} y={y + height / 2 - 7} width={14} height={14} viewBox="0 0 16 16" fill="#cc3340">
...
function Dashboard({ state, onStateChange }) {
            ...
            <AlertWrapper>
              <Alert type="warning">
                <AlertRow>
                  ...
                  <Button size="small" onClick={() => setShowRecommendations(true)}>View recommendations</Button>
                </AlertRow>
        ...
        <DashboardGrid>
          <Section>
            ...
            <Row>
              ...
              <MetricCard $status="critical" style={{ flex: 1 }}>
                <AlertIconBadge>
                ...
                <RankedList>
                  {escalationData.map((entry, index) => (
                    <RankedListItem key={index}>
                      <RankedListLeft>
                        <RankedListDot $critical={entry.critical} />
                        <RankedListLabel>
                          {entry.intent}
                        </RankedListLabel>
                        <RankedListBar>
                          <RankedListBarFill $critical={entry.critical} $percent={entry.rate} />
                        </RankedListBar>
                      </RankedListLeft>
                      <RankedListValue $critical={entry.critical}>
                        {entry.rate}%
                        {entry.critical && (
                          <InlineAlertIcon>
                            <svg viewBox="0 0 16 16" fill="currentColor">
            ...
            <Row>
              <ChartCard style={{ flex: '0 0 45%' }}>
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={queueDepthData} layout="vertical" margin={{ left: 0, right: 60 }} barCategoryGap="25%">
                    ...
                    <YAxis type="category" dataKey="workstream" width={140} axisLine={false} tickLine={false} fontSize={12} />
                    <Bar dataKey="count" radius={[4, 4, 4, 4]} background={{ fill: '#e9ebed', radius: 4 }}>
                      {queueDepthData.map((entry, index) => (
                        <Cell key={index} fill={entry.critical ? '#cc3340' : '#3694E2'} />
                      ))}
              ...
              <MetricCardsColumn $stretch style={{ flex: 1 }}>
                ...
                <MetricCard>
                  ...
                  <MetricValue>{metrics.agentAvailability}</MetricValue>
                  <MetricSubtext>33% spare capacity</MetricSubtext>
                </MetricCard>
              </MetricCardsColumn>
